...
svgRoot.addEventListener("click", (ev) => {
  ...
  importBtn.addEventListener("click", () => {
    // IMPORTANT: file picker must be opened directly from the click gesture (Safari/Chrome strictness),
    // so we prompt for password AFTER the user selects a file.
    ...
    inp.onchange = async () => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const m = line.match(/^([^,]+),([^,]+),"(.*)"$/);
        if (!m) continue;
        out.push({ standId: normStandId(m[1]), status: m[2].trim().toLowerCase(), company: m[3].replaceAll('""','"') });
      }
      rows = out;
      applyColours();
      renderTable();
      // Import changes are local until you click Save on a stand (keeps Sheets as source of truth).
    };
    inp.click();
  });
